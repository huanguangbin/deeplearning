...
class OctAug:
    def __init__(self, vol_data, vol_mask):
        self.vol_data = vol_data
        self.vol_mask = vol_mask
    ...
    def aug(self):
        if round(np.random.uniform(0, 1), 1) <= 0.2:
            for idx, img_patch in enumerate(self.vol_data):
                msk_patch = self.vol_mask[idx]
                self.vol_data[idx], self.vol_mask[idx] = self.rotate(img_patch, msk_patch, self.rotate_angle)
        if round(np.random.uniform(0, 1), 1) <= 0.2:
            for idx, img_patch in enumerate(self.vol_data):
                msk_patch = self.vol_mask[idx]
                self.vol_data[idx], self.vol_mask[idx] = self.blur(img_patch, msk_patch)
        if round(np.random.uniform(0, 1), 1) <= 0.2:
            for idx, img_patch in enumerate(self.vol_data):
                msk_patch = self.vol_mask[idx]
                self.vol_data[idx], self.vol_mask[idx] = self.shifit(img_patch, msk_patch, self.shifit_limit,
                                                                     self.shifit_dx, self.shifit_dy)
        if round(np.random.uniform(0, 1), 1) <= 0.2:
            for idx, img_patch in enumerate(self.vol_data):
                msk_patch = self.vol_mask[idx]
                self.vol_data[idx], self.vol_mask[idx] = self.random_filp(img_patch, msk_patch, self.flip_axis)
        if round(np.random.uniform(0, 1), 1) <= 0.1:
            for idx, img_patch in enumerate(self.vol_data):
                msk_patch = self.vol_mask[idx]
                self.vol_data[idx], self.vol_mask[idx] = self.add_noise(img_patch, msk_patch)
        if round(np.random.uniform(0, 1), 1) <= 0.2:
            for idx, img_patch in enumerate(self.vol_data):
                msk_patch = self.vol_mask[idx]
                self.vol_data[idx], self.vol_mask[idx] = self.contrast_adjust(img_patch, msk_patch)
        return self.vol_data, self.vol_mask

    def norm_aug(self):
        self.aug()
        mean = 0.3383
        std = 0.1146  # total-------mean:0.3383131224320247----std:0.1146382573485248
        max_voxel = 255.
        ...
        self.vol_data = self.vol_data / max_voxel
        if round(np.random.uniform(0, 1), 1) <= 0.2:  # 伽马变化
            self.vol_data = np.power(self.vol_data, self.gamma_coef)
        self.vol_data = (self.vol_data - mean) / std
        return self.vol_data, self.vol_mask
