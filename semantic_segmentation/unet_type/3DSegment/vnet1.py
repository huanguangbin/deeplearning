...
class ConvLayer(nn.Module):
    def __init__(self, num_channels, prelu, dilation):
        ...
        self.conv1 = nn.Conv3d(num_channels, num_channels, kernel_size=5, padding=dilation * 2, dilation=dilation,
                               padding_mode='zeros')
...
class InputTransition(nn.Module):
    def __init__(self, out_channels, prelu, dilation=1, attention_module=False):
        ...
        self.conv1 = nn.Conv3d(1, out_channels, kernel_size=5, padding=dilation * 2, dilation=dilation,
                               padding_mode='zeros')
...
class SpatialGate(nn.Module):
    def __init__(self, in_channels, reduction_ratio=4, dilated_conv_layers=2, dilation=4):
        ...
        for i in range(dilated_conv_layers):
            self.spatial_gate_layers.append(
                nn.Conv3d(in_channels // reduction_ratio, in_channels // reduction_ratio, kernel_size=3,
...
class VNet(nn.Module):
    ...
    def __init__(self, num_layers=1, prelu=True, dilation=1, attention_module=False):
        ...
        self.input_transition = InputTransition(16, prelu, dilation, attention_module)
        ...
        self.down_transition128 = DownTransition(64, num_layers + 2, prelu, dilation, attention_module)
        self.down_transition256 = DownTransition(128, num_layers + 1, prelu, dilation, attention_module)
        self.up_transition256 = UpTransition(256, 256, num_layers + 1, prelu)
        self.up_transition128 = UpTransition(256, 128, num_layers + 1, prelu)
        self.up_transition64 = UpTransition(128, 64, num_layers, prelu)
        self.up_transition32 = UpTransition(64, 32, num_layers, prelu)
        self.out_transition = OutputTransition(32, prelu)

    def forward(self, x):
        out16 = self.input_transition(x)
        out32 = self.down_transition32(out16)
        out64 = self.down_transition64(out32)
        out128 = self.down_transition128(out64)
        out256 = self.down_transition256(out128)
        out = self.up_transition256(out256, out128)
        out = self.up_transition128(out, out64)
